#ifdef __linux__
// Define a macro para limpar a tela no Linux
#define clear "clear"
#elif WIN_32
// Define a macro para limpar a tela no Windows
#define clear "cls"
#endif
...
// Estrutura para armazenar um array de inteiros
typedef struct numArray {
    int32_t *data; // Ponteiro para os dados
...
// Função para ordenar um array usando o método Bubble Sort
...
// Função para ordenar um array usando o Bubble Sort melhorado
void bubbleSortImprov(numArray* buffer) {
    unsigned short switched = 1; // Indica se houve troca
    size_t index_aux = buffer->size - 1, index_limit = buffer->size - 1, j;

    while (switched) {
        j = 0, switched = 0; // Reinicia o estado da troca
        while (j < index_limit) {
            if (buffer->data[j] > buffer->data[j + 1]) {
                trocar(&buffer->data[j], &buffer->data[j + 1]);
                switched = 1; // Houve troca
                index_aux = j; // Atualiza o índice onde ocorreu a troca
            }
            j++;
        }
        index_limit = index_aux; // Atualiza o limite para a próxima iteração
    }
}
...
void insertionSort(numArray* buffer) {
    ...
    for (size_t i = 1; i < buffer->size; ++i) {
        ...
        while ((j >= 0) && (key < buffer->data[j])) {
            buffer->data[j + 1] = buffer->data[j]; // Move o elemento para a direita
            --j;
        }
        buffer->data[j + 1] = aux; // Insere o elemento na posição correta
    }
}
...
void merge(numArray* buffer, size_t esq, size_t center, size_t dir) {
    ...
    int32_t *tempBuffer;
    tempBuffer = malloc((dir - esq + 1) * sizeof(int32_t)); // Aloca buffer temporário
    ...
    free(tempBuffer); // Libera o buffer temporário
}
...
// Função para escolher um pivô para o Quick Sort
void pivot(unsigned short esc, numArray* buffer, size_t ini, size_t end, size_t *pivo) {
    ...
    switch (esc) {
        case 1: // Pivô aleatório
            *pivo = ini + rand() % (end - ini + 1);
            break;
        case 2: // Pivô como último elemento
            *pivo = end;
            break;
        case 3: // Pivô como mediana de três
            meio = (ini + end) / 2;
...
void quickSort(numArray* buffer, int ini, int end, unsigned short esc) {
    ...
    pivot(esc, buffer, ini, end, &pivo); // Escolhe o pivô
    auxKey = buffer->data[pivo]; // Guarda o valor do pivô
    trocar(&buffer->data[pivo], &buffer->data[end]); // Move o pivô para o final
    ...
    while (i <= j) {
        ...
        if (i < j) {
            trocar(&buffer->data[i], &buffer->data[j]); // Troca os elementos
            i++;
            j--;
        }
    }
    trocar(&buffer->data[i], &buffer->data[end]); // Coloca o pivô na posição correta
...
void descer(numArray* buffer, int n, int i) {
    ...
    if (right < n && buffer->data[right] > buffer->data[maior])
        maior = right;
    if (maior != i) {
        trocar(&buffer->data[i], &buffer->data[maior]); // Troca o pai com o maior filho
        descer(buffer, n, maior); // Recursão para descer na árvore
    }
}

// Função para arranjar o Heap
void arranjar(numArray* buffer, int n) {
    for (int i = n / 2 - 1; i >= 0; --i) {
        descer(buffer, n, i); // Chama a função de descer para cada nó
    }		
}
...
void heapSort(numArray* buffer) {
    int n = buffer->size;
    arranjar(buffer, n); // Arranja o Heap
    while (n - 1 > 0) {
        trocar(&buffer->data[0], &buffer->data[n - 1]); // Troca o primeiro elemento com o último
        n--;
        descer(buffer, n, 0); // Desce a árvore do Heap
    }
}
